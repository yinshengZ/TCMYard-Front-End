  ...
  <div>
    <el-form>
      <el-form-item label="Patient ID:">
        <el-input v-model="this.id" disabled />
      </el-form-item>
      ...
      <el-form-item label="Disease:">
        ...
        <el-button @click="load_add_disease_form"> <svg-icon icon-class="add" /></el-button>
      </el-form-item>
    ...
    <el-dialog :visible.sync="add_disease_form_loaded" :before-close="get_all_diseases" width="30%" append-to-body>
      <add-disease />
  ...
  methods: {
    get_all_diseases() {
      getDiseases().then((response) => {
        this.all_diseases = response.data
