  ...
  methods: {
    ...
    get_current_day_incomes() {
      getCurrentDailyIncomes().then((response) => {
        ...
        for (let i = 0; i < response.data.length; i++) {
          value.push(response.data[i].amount / 100)
          date.push(i + 1)
        }
    ...
    get_current_day_daily_income_distribution() {
      getCurrentDayDailyIncomeDistribution().then((response) => {
        ...
        if (response.data.length > 0) {
          for (let i = 0; i < response.data.length; i++) {
            const data = {}
            data.value = response.data[i].amount
            data.name = uppercaseFirst(response.data[i].service['categories'])
    ...
    get_current_week_daily_incomes() {
      getCurrentWeekDailyIncomes().then((response) => {
        const title = 'Current Week Daily Incomes'
        const final_data = {}
        const value = []
        const date = []
    ...
    get_current_week_income_distribution() {
      getCurrentWeekIncomeDistribution().then((response) => {
        const final_data = []
        const title = 'Current Week Incomes By Category'

        for (let i = 0; i < response.data.length; i++) {
          const data = {}
          data.value = response.data[i].amount / 100
          data.name = response.data[i].service['categories']
          final_data.push(data)
        }
    ...
    get_current_month_daily_incomes() {
      getCurrentMonthDailyIncomes().then((response) => {
        const title = 'Current Month Daily Incomes'
        const final_data = {}
        const value = []
        const date = []
    ...
    current_year_incomes() {
      getCurrentYearIncomes().then((response) => {
        this.year_income = Number(response.data) / 100
      })
    },

    current_month_incomes() {
      getCurrentMonthIncomes().then((response) => {
        this.month_income = Number(response.data) / 100
      })
    },

    current_week_incomes() {
      getCurrentWeekIncomes().then((response) => {
        this.week_income = Number(response.data) / 100
      })
    },

    current_day_incomes() {
      getCurrentDayIncomes().then((response) => {
        this.day_income = Number(response.data) / 100
      })
    }
  }
}
