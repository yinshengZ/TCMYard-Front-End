<template>
  <div class="container">
    ...
    <div class="chart-area">
      <el-card>
        <div id="YOY-Barchart" class="YOY-Barchart" />
      </el-card>

      <el-card>
        <div id="YOY-PieChart" class="YOY-PieChart" />
      </el-card>
    </div>
  ...
  methods: {
    ...
    get_weekly_comparisons() {
      getWeeklyComparisonIncomes().then((response) => {
        ...
        for (let i = 0; i < response.data.length; i++) {
          ...
          date.push(response.data[i].week)
        }
    ...
    get_monthly_comparisons() {
      getMonthlyComparisonIncomes().then((response) => {
        ...
        for (let i = 0; i < response.data.length; i++) {
          value.push(response.data[i].amount / 100)
          date.push(response.data[i].year + '-' + response.data[i].month)
        }
    ...
    get_yearly_comparisons() {
      getYearlyComparisonIncomes().then((response) => {
        ...
        for (let i = 0; i < response.data.length; i++) {
          value.push(response[i].amount / 100)
      ...
      this.chart.on('click', params => {
        if (params.seriesName == 'yearly') {
          getYearDistributions(params.name).then((response) => {
            ...
            for (let i = 0; i < response.data.length; i++) {
              const data = {}
              const service = response.data[i].service
