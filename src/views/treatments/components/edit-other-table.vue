<template>
  <div>
    <div>
      <el-button class="add-other-button" type="success" icon="el-icon-plus" @click="add_item_row">Other</el-button>
    </div>
      ...
      <div v-for="(inventory, index) in treatment_detail.inventories" :key="inventory.id">
        <el-form-item :label="'Item' + (index + 1)">
          <div class="grid-container">
            ...
            <div class="units">
              <el-input-number v-model="inventory.pivot.units" :min="1" :step="1" />
            </div>
    ...
    calculate_price() {
      ...
      if (this.treatment_detail.inventories.length > 0) {
        for (let i = 0; i < this.treatment_detail.inventories.length; i++) {
          const result = this.all_others.find(item => item.id === this.treatment_detail.inventories[i].pivot.inventory_id)
